...
export const ga4Phase3: Lesson[] = [
  {
    ...
    description: `
### 📘 개념 학습: 가입 경로(Method)의 중요성
회원가입은 사용자가 우리 서비스에 **"내 정보를 줘도 좋다"**고 허락한 중요한 순간입니다.
마케터는 사용자가 **"카카오로 가입했는지, 이메일로 가입했는지"** 매우 궁금해합니다. 왜냐하면 '카카오 간편가입'을 도입했을 때 가입률이 얼마나 올랐는지 증명해야 하기 때문입니다.
...
이렇게 되면 **신규 회원 수가 실제보다 부풀려져서** 마케팅 성과가 왜곡됩니다.

*   **sign_up**: 평생 딱 한 번만 발생 (신규 유입)
*   **login**: 방문할 때마다 발생 (유저 리텐션/충성도)
...
퍼널(Funnel) 분석에서 "조회 대비 장바구니 전환율"을 계산하려면, 상품 ID가 조회 단계와 장바구니 단계에서 **동일**해야 하기 때문입니다.
